...
async def api_version(force: bool = False):
    """Eigene Version und – falls hinterlegt – ob es eine neuere auf GitHub gibt."""
    from . import version as V
    cfg = C.load()
    if not cfg.get("update_check", True) or not cfg.get("update_repo"):
        return {"ok": True, "version": V.VERSION, "latest": None, "update": False,
                "repo": cfg.get("update_repo", "")}
    out = await V.check(cfg.get("update_repo", ""), force=force)
    return {**out, "repo": cfg.get("update_repo", "")}
