...
def _swap(new_app: Path) -> None:
    ...
    Alles geschieht **innerhalb** von ``app``: Der neue Stand wird nach
    ``app/.update-new`` gelegt, der alte nach ``app/.update-old`` geschoben,
    dann rückt der neue an seine Stelle. Umbenennen im selben Ordner ist
    unteilbar und schnell.

    Warum nicht einfach den ganzen Ordner umbenennen? Weil ``app`` in vielen
    Aufbauten ein eigener Einhängepunkt ist (Docker-Volume, Bind-Mount). Ein
    Umbenennen über eine Dateisystemgrenze scheitert mit „Invalid cross-device
    link“. Innerhalb des Ordners kann das nicht passieren.
    """
    keep = {".update-new", ".update-old", "__pycache__"}
    staged = APP_DIR / ".update-new"
    old = APP_DIR / ".update-old"
    shutil.rmtree(staged, ignore_errors=True)
    shutil.rmtree(old, ignore_errors=True)

    # 1) Neuen Stand vollständig bereitlegen. Fehler hier kosten nichts.
    shutil.copytree(new_app, staged, ignore=shutil.ignore_patterns("__pycache__"))
    ...
    # 2) Sicherung der laufenden Fassung (nur, wenn sie gelingt).
    if BACKUP.exists():
        shutil.rmtree(BACKUP, ignore_errors=True)
    BACKUP.parent.mkdir(parents=True, exist_ok=True)
    try:
        shutil.copytree(APP_DIR, BACKUP,
                        ignore=shutil.ignore_patterns("__pycache__", ".update-*"))
    except OSError:
        pass        # Eine fehlende Sicherung darf das Update nicht verhindern

    # 3) Alten Stand beiseite schieben.
    old.mkdir()
    moved: list[str] = []
    for item in list(APP_DIR.iterdir()):
        if item.name in keep:
            continue
        os.rename(item, old / item.name)
        moved.append(item.name)

    # 4) Neuen Stand einrücken. Klemmt es, kommt der alte zurück.
    try:
        for item in list(staged.iterdir()):
            os.rename(item, APP_DIR / item.name)
    except OSError:
        for item in list(APP_DIR.iterdir()):
            if item.name in keep:
                continue
