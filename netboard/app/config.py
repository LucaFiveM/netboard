...
DEFAULTS: dict[str, Any] = {
    ...
    "users": [],
    # GitHub-Repository für die Update-Prüfung, z. B. "benutzer/netboard".
    "update_repo": "",
    "update_check": True,
...
def validate(patch: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
    ...
    if "_fontv" in patch:
        try:
            ...
            pass
    if "update_repo" in patch:
        rp = str(patch["update_repo"] or "").strip().strip("/")
        if rp and not re.fullmatch(r"[\w.-]+/[\w.-]+", rp):
            errs.append("Repository bitte als „benutzer/projekt“ angeben.")
        else:
            clean["update_repo"] = rp[:80]
